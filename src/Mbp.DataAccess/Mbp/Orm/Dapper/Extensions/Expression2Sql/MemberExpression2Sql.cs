...
using WuhanIns.Nitrogen.Ddd.Extensions;
using System;
...
namespace WuhanIns.Nitrogen.Orm.Dapper.Extensions.Expression2Sql
{
	class MemberExpression2Sql : BaseExpression2Sql<MemberExpression>
	{
		...
		protected override SqlPack Where(MemberExpression expression, SqlPack sqlPack)
		{
			...
			{
				//实体字段访问
				string tableName = sqlPack.GetTableAlias(PropertyInfoCache.GetTableName(expression.Expression.Type.FullName));
				if (!string.IsNullOrWhiteSpace(tableName))
				{
					tableName += ".";
				}
				sqlPack += " " + tableName + expression.Member.Name;
			}
			else if (expression.ToString() == "String.Empty")
			{
				//过滤引用类型String string.Empty,其他自定义的引用类型用null禁止使用自己实现的Empty成员
				sqlPack.AddDbParameter("");
			}
			...
			else
			{
				...
				sqlPack.AddDbParameter(t.GetValueTypeDefaultValue());
			}
			return sqlPack;
		}
		...
		{
			return GroupBy(expression, sqlPack) + " ASC";
		}
		...
		/// <returns></returns>
		protected override SqlPack OrderByDescending(MemberExpression expression, SqlPack sqlPack)
		{
			return GroupBy(expression, sqlPack) + " DESC";
		}
