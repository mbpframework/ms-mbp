...
namespace WuhanIns.Nitrogen.Orm.Dapper.Extensions.Expression2Sql
{
    class MethodCallExpression2Sql : BaseExpression2Sql<MethodCallExpression>
    {
        ...
        private static void Property(MethodCallExpression expression, SqlPack sqlPack)
        {
            string tableName = sqlPack.GetTableAlias(PropertyInfoCache.GetTableName(expression.Arguments[0].Type.FullName));
            if (!string.IsNullOrWhiteSpace(tableName))
            {
                tableName += ".";
            }
            sqlPack += " " + tableName + (expression.Arguments[1] as ConstantExpression).Value;

            sqlPack += " ";

        }

        public new static void In(MethodCallExpression expression, SqlPack sqlPack)
        {
